...
class PDFProcessor:
    ...
    def _normalize_for_match(self, text):
        ...
        text = text.replace('์า', 'ำ')  # ์า (2 chars) -> ำ (U+0E33) -- PDF encoding quirk
        text = re.sub(r'["\'\'\u201c\u201d\*\[\]\(\)]+', '', text)  # strip quotes/brackets that PDFs add
    ...
    def add_labels_to_pdf(self, input_pdf, output_pdf, orders_to_label):
        ...
        date_str = f"{now.day}/{now.month}({now.strftime('%H:%M')})"
        ...
        for i, page in enumerate(reader.pages):
            ...
            if page_num in orders_to_label:
                ...
                # --- Date overlay at top-left corner ---
                date_font_size = 22
                if self.font_path:
                    try:
                        pdfmetrics.registerFont(TTFont('ThaiFont', self.font_path))
                        can.setFont('ThaiFont', date_font_size)
                    except: can.setFont('Helvetica-Bold', date_font_size)
                else: can.setFont('Helvetica-Bold', date_font_size)
                date_w = can.stringWidth(date_str, can._fontname, date_font_size)
                date_y = (h / 2) + 20  # Slightly above center
                can.setFillColorRGB(1, 1, 1)  # White background
                can.rect(3, date_y - 3, date_w + 10, date_font_size + 6, fill=1, stroke=0)
                can.setFillColorRGB(1, 0, 0)  # Red text
                can.drawString(8, date_y, date_str)
                ...
                for zone_key, zone_items in zones.items():
                    ...
                    # Calculate bigger font size for quantity numbers
                    qty_font_size = min(font_size + 6, 28)  # qty is 6pt bigger, max 28
                    ...
                    for d in reversed(all_lines_data):
                        ...
                        # Draw the code text part (without the qty)
                        qty_str = f"x{d['qty']}"
                        code_part = d['text'][:d['text'].rfind(qty_str)] if qty_str in d['text'] else d['text']
                        
                        # Draw code portion at normal font size
                        can.setFont(can._fontname, font_size)
                        can.drawString(rx + 5, curr_y, code_part)
                        code_w = can.stringWidth(code_part, can._fontname, font_size)
                        
                        # Draw qty portion (x1, x2, etc.) at BIGGER font size
                        if qty_str in d['text']:
                            can.setFont(can._fontname, qty_font_size)
                            # Adjust y to vertically center the bigger text relative to the line
                            qty_y_offset = (qty_font_size - font_size) * 0.35
                            can.drawString(rx + 5 + code_w, curr_y - qty_y_offset, qty_str)
                            can.setFont(can._fontname, font_size)  # Reset
                        
                        if d['qty'] > 1:
                            can.setStrokeColorRGB(1, 0, 0); can.setLineWidth(2.0)
                            qw = can.stringWidth(qty_str, can._fontname, qty_font_size)
                            cx = rx + 5 + code_w + (qw / 2)
                            cy = curr_y + (font_size * 0.35)
                            can.circle(cx, cy, qty_font_size * 0.65, fill=0, stroke=1)
                        curr_y += line_h
